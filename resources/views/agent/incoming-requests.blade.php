    ...
    <div class="col-lg-8">
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body border-bottom">
                <form method="GET" action="{{ route('agent.incoming-requests') }}" class="d-flex mb-3">
                    <input type="text" name="ref" value="{{ request('ref') }}" class="form-control form-control-sm me-2" placeholder="Search by reference code (exact)">
                    <button type="submit" class="btn btn-sm btn-primary">Search</button>
                    @if(request()->has('ref') && request('ref') !== '')
                        <a href="{{ route('agent.incoming-requests') }}" class="btn btn-sm btn-outline-secondary ms-2">Clear</a>
                    @endif
                </form>
            </div>
            <div class="card-body">
                            ...
                            <div class="card-body">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-6 text-end">
                                        ...
                                        <p class="text-muted small mb-1">Ref: {{ $request->reference_code }}</p>
                                        <p class="small mb-0">
                                            <strong>Requested:</strong>
                                            {{ number_format($request->amount_sent, 2) }}
                                        </p>
                                        <p class="small mt-1">
                                            <small class="text-muted">Your Balance: {{ number_format($agentProfile->cash_balance, 2) }}</small>
                                        </p>
                                    </div>
                                </div>
                ...
                @else
                    <div class="alert alert-info text-center py-4">
                        <i data-lucide="search" style="width: 32px; height: 32px; display: inline-block; margin-bottom: 8px;"></i>
                        <p class="mb-2"><strong>Pending transfer requests are hidden.</strong></p>
                        <p class="mb-0">Enter an exact reference code above to locate a specific pending transfer.</p>
                    </div>
                @endif
            </div>
        </div>
